...
public class RightMallet : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (startTimer)
        {
            ...
            if (Input.GetKeyDown(KeyCode.D) || Input.GetKeyDown(KeyCode.RightArrow) && startTimer)
            {
                timesCanBeUsed += 1;
            }
    ...
    void FixedUpdate()
    {
        if (Input.GetKey(KeyCode.D) && canPush && malletEnabled || Input.GetKey(KeyCode.RightArrow) && canPush && malletEnabled)
        {
            timesCanBeUsed -= 1;
            startDown = true;
            canPush = false;
